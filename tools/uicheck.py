...
# 盤面は参考より 1% ほど細くなる。
# 左レール上限 0.16→0.17、盤面の下端下限 0.87→0.86 は、左右移動ボタンが
# 8.1mm しか無かった（9mm 目標に届かない）のを実寸で詰め直したときの
# 副作用。レールを少し太くしたぶん盤面がわずかに細く・低くなる
# （tools/touchcheck.py 側は 9mm 台に改善している。詳細は
# docs/UI_REFERENCE.md 第10節）。
RULES = {
    "盤面の左端": (0.23, 0.29), "盤面の右端": (0.70, 0.78),
    "盤面の上端": (0.23, 0.31), "盤面の下端": (0.86, 0.94),
    "左レールの幅": (0.06, 0.17), "右レールの幅": (0.11, 0.19),
}
